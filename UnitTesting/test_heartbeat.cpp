                ...
                std::thread([this, client_fd]() {
                    ...
                    while (server_running) {
                        ...
                        if (hb.ParseFromArray(buffer, bytes)) {
                            ...
                            auto latency = std::chrono::duration_cast<std::chrono::milliseconds>(now - timestamp).count();
                            
                            std::cout << "Received heartbeat from server_id: " << hb.server_id()
                                      << ", IP: " << hb.ip()
...
TEST_F(HeartbeatTest, TestMultipleHeartbeats) {
    ...
    EXPECT_GE(received_heartbeats, NUM_HEARTBEATS) 
        << "Expected at least " << NUM_HEARTBEATS 
        << " heartbeats, but got " << received_heartbeats;
}
