#include "../../generated/heat_beat.pb.h"
#include "../include/file_chunks.hpp"
#include "../include/heart_beat.hpp"
#include "../include/machine_details.hpp"
...
class ClusterServer
{
      private:
        ...
        std::map<std::string, std::vector<std::string>>
            replication_groups_; // chunk_id -> replica_servers
      ...
      public:
        ClusterServer(const std::string &server_id, const std::string &storage_path, short port,
              ...
              replication_factor_(replication_factor), running_(false)
        {
                ...
                std::cout << "Cluster server " << server_id_ << " initialized on port " << port
                          << std::endl;
      ...
      private:
        ...
        void send_heartbeat()
        {
                try
                {
                        ...
                        if (resources.cpu_usage > 80 || resources.memory_usage > 80 ||
        ...
        bool store_chunk(const std::string &chunk_id, const std::vector<uint8_t> &data)
        {
                try
                {
                        ...
                        if (!file)
                        {
                                std::cerr << "Failed to create chunk file: " << chunk_path
                                          << std::endl;
                                return false;
                        }
        ...
        std::vector<uint8_t> retrieve_chunk(const std::string &chunk_id)
        {
                ...
                std::cout << "Retrieved chunk " << chunk_id << " (" << size << " bytes)"
                          << std::endl;
        ...
        class Session : public std::enable_shared_from_this<Session>
        {
              ...
              private:
                ...
                void handle_request(const std::string &request_data)
                {
                        ...
                        boost::asio::async_write(socket_, boost::asio::buffer(request_data),
...
int main(int argc, char *argv[])
{
        if (argc != 4)
        {
                std::cerr << "Usage: " << argv[0] << " <server_id> <storage_path> <port>"
                          << std::endl;
                return 1;
        }
